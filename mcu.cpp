	...
	while(!confFileStream.atEnd())
	{
		...
		if(stringLine.contains(QString("\"").append(id).append("\""),Qt::CaseInsensitive))
		{
			while (!stringLine.contains(";",Qt::CaseInsensitive) || stringLine.length() > 1)
			{
				...
				if(stringLine.contains("desc",Qt::CaseInsensitive) && description.isNull())
				{
					stringLine.remove(" ");
					stringLine.remove("	");
				...
				else if(stringLine.contains("signature",Qt::CaseInsensitive) && signature.isNull())
				{
					stringLine.remove("	");
					stringLine.remove("desc");
					stringLine.remove("=");
